...
log "🧩 Adding fsck.mode=skip autoinstall parameters to kernel command line..."
sed -i -e 's/---/ fsck.mode=skip autoinstall  ---/g' "$tmpdir/isolinux/txt.cfg"
sed -i -e 's/---/ fsck.mode=skip autoinstall  ---/g' "$tmpdir/boot/grub/grub.cfg"
sed -i -e 's/---/ fsck.mode=skip autoinstall  ---/g' "$tmpdir/boot/grub/loopback.cfg"
